...
elSelect.addEventListener("change", (evt) => {
  ...
  if (value === "all") {
    renderProduct(galleryarray);
  } else {
    const newProducts = galleryarray.filter((product) => {
      return product.postId === Number(value);
    });
...
elCards.addEventListener("click", (evt) => {
  ...
  if (target.className.includes("delete-btn")) {
    ...
    galleryarray.forEach((product) => {
      if (product.id !== id) {
        newProducts.push(product);
      }
    });
    galleryarray = newProducts;
    renderProduct(galleryarray);
  }
});
...
function renderProduct(currarray) {
  document.getElementById("card").innerText = "";

  for (var i = 0; i < currarray.length; i++) {
    document.getElementById("card").innerHTML += `
		<div class="col-md-4 mt-3">
		<div class="card p-3 ps-5 pe-5 border-">
		 <h4 class="text-capitalize text-center text-info card-title">${currarray[i].name}</h4>
		 <h5 class="text-capitalize text-center text-white">${currarray[i].email}</h5>
		 <p class="mt-2 text-light">${currarray[i].body}</p>
		 <button class="btn w-100 mx-auto delete-btn bg-danger text-light" id="${currarray[i].id}">Delete</button>
		</div>
		</div>
		`;
  }
